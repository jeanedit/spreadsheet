...
void TestFormulaReferences() {
    ...
    ASSERT_EQUAL(evaluate("A1+B3"), 1);  // Cell with empty text
    ASSERT_EQUAL(evaluate("A1+B1"), 1);  // Empty cell
    ASSERT_EQUAL(evaluate("A1+E4"), 1);  // Cell outside of the table boundaries
}
...
void TestCellReferences() {
    ...
    // Reference to a cell outside of the table boundaries
